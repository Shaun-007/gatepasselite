...
* SQLite
* Node.js built-in `node:sqlite` for local development
* Turso Cloud (`@libsql/client`) on the hosted backend
...
Local development uses SQLite (Node's built-in `node:sqlite`) automatically with zero external setup.
